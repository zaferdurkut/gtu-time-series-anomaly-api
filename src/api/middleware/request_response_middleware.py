...
from firefly_logger.log_provider import LOG_IS_ENABLED, LOG_TRACE_ID_HEADER_NAME, \
    setup_trace_id, logger
...
class RequestResponseMiddleware(BaseHTTPMiddleware):
    ...
    def process_request(request: Request):
        ...
        def get_trace_id():
            if LOG_TRACE_ID_HEADER_NAME in request.headers and request.headers[LOG_TRACE_ID_HEADER_NAME] is not None:
    ...
    def process_response(request: Request, response: Response):
        ...
        if http_status_code >= 400:
            if http_status_code >= 500:
                logger.fatal(
                    str.format(log_template, request.url.path, request.method, http_status_code, elapsed_ms),
                    log_data)
            else:
                logger.error(
                    str.format(log_template, request.url.path, request.method, http_status_code, elapsed_ms),
